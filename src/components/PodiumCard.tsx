...
import { Trophy, Medal, Timer, Zap, CheckCircle2, ChevronDown, ChevronUp, Sparkles } from 'lucide-react';
...
export const PodiumCard: React.FC<PodiumCardProps> = ({
  ...
  useEffect(() => {
    // Fire celebratory confetti for the 1st place winner
    try {
      confetti({
        particleCount: 50,
        spread: 60,
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {evaluations.slice(0, 3).map((ev, index) => {
          ...
          return (
            <div
              key={ev.modelId}
              onClick={() => onSelectModel(ev.modelId)}
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between border-t border-zinc-800/80 pt-2.5 text-[11px] text-zinc-400">
                ...
                <span className="font-medium text-indigo-400 hover:underline">
                  {isSelected ? 'Viewing' : 'Inspect Answer →'}
                </span>
              </div>
            </div>
          );
